  ...
  return (
    <div className="p-6 bg-[#090d16] text-slate-100 min-h-screen">
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-extrabold tracking-tight text-white">Marketing & CRM</h1>
        ...
        <button
          ...
          className="inline-flex items-center gap-1.5 rounded-xl bg-[#00e699] hover:bg-[#05df8a] px-4 py-2 text-sm font-bold text-slate-950 shadow-lg shadow-emerald-500/20 transition-all hover:-translate-y-0.5">
      ...
      {/* Tabs */}
      <div className="mb-6 border-b border-slate-800/80">
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  isActive
                    ? 'border-[#00e699] text-[#00e699]'
                    : 'border-transparent text-slate-400 hover:text-slate-200'
                )}
              >
                <Icon className={cn("h-4 w-4", isActive ? "text-[#00e699]" : "text-slate-400")} />
                {tab.label}
              </button>
            )
          })}
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          {/* Campanhas */}
          {activeTab === 'campaigns' && (
            <div className="rounded-xl border border-slate-800/80 bg-[#0f1422]">
              <div className="flex items-center justify-between border-b border-slate-800/80 px-6 py-4">
                ...
                <button
                  onClick={() => setShowNewCampaign(true)}
                  className="inline-flex items-center gap-1.5 rounded-lg bg-[#00e699] hover:bg-[#05df8a] px-3 py-1.5 text-xs font-bold text-slate-950 shadow-md shadow-emerald-500/20">
                        ...
                        <tr>
                          <td colSpan={6} className="px-4 py-14">
                            <div className="flex flex-col items-center text-center">
                              ...
                              <p className="mb-1 font-semibold text-white">Sem campanhas</p>
                              <p className="mb-4 text-sm text-slate-400">Crie a primeira campanha de email para os seus leads.</p>
                              <button
                                onClick={() => setShowNewCampaign(true)}
                                className="inline-flex items-center gap-1.5 rounded-lg bg-[#00e699] hover:bg-[#05df8a] px-4 py-2 text-xs font-bold text-slate-950"
                              >
          ...
          {activeTab === 'calendar' && (
            <div className="rounded-xl border border-slate-800/80 bg-[#0f1422] p-6">
              <div className="mb-6 flex items-center justify-between">
                ...
                <button
                  ...
                  className="inline-flex items-center gap-1.5 rounded-lg bg-[#00e699] hover:bg-[#05df8a] px-3.5 py-2 text-xs font-bold text-slate-950 shadow-md shadow-emerald-500/20 transition-all">
                  <Plus className="h-3.5 w-3.5 stroke-[2.5]" /> Novo Evento
                </button>
              </div>

              <div className="grid grid-cols-7 gap-px overflow-hidden rounded-lg border border-slate-800/80 bg-slate-800/40">
                {['Dom', 'Seg', 'Ter', 'Qua', 'Qui', 'Sex', 'Sáb'].map((d) => (
                  <div key={d} className="bg-[#070a11] px-2 py-2 text-center text-xs font-bold text-slate-400 uppercase tracking-wider">
                    {d}
                  </div>
                ))}
                {Array.from({ length: firstDay }).map((_, i) => (
                  <div key={`empty-${i}`} className="min-h-24 bg-[#090d16] p-1" />
                ))}
                {Array.from({ length: daysInMonth }, (_, i) => i + 1).map((day) => {
                  const dayEvents = calendarEvents.filter((e) => e.day === day)
                  const isToday = day === today.getDate()
                  return (
                    <div key={day} className={cn('min-h-24 bg-[#090d16] p-1.5 transition-colors', isToday && 'ring-2 ring-inset ring-[#00e699] bg-[#00e699]/5')}>
                      <div className={cn('mb-1 inline-flex h-6 w-6 items-center justify-center rounded-full text-xs font-bold', isToday ? 'bg-[#00e699] text-slate-950 shadow-[0_0_8px_rgba(0,230,153,0.4)]' : 'text-slate-300')}>
                        {day}
                      </div>
                      <div className="space-y-1">
                        {dayEvents.map((event) => (
                          <div key={event.id} className={cn('truncate rounded px-1.5 py-0.5 text-[10px] font-semibold', event.color)}>
                            {event.title}
                          </div>
                        ))}
                      </div>
              ...
              {calendarLoading && (
                <p className="mt-3 text-center text-xs text-slate-400">A carregar eventos...</p>
              )}
              {!calendarLoading && calendarEvents.length === 0 && (
                <p className="mt-3 text-center text-xs text-slate-400">Sem eventos agendados este mês.</p>
              )}
            </div>
          )}
          ...
          {activeTab === 'seo' && (
            <div className="rounded-xl border border-slate-800/80 bg-[#0f1422]">
              <div className="flex items-center justify-between border-b border-slate-800/80 px-6 py-4">
                <h3 className="font-semibold text-white">SEO Tracker</h3>
                <button
                      ...
                      {(keywords ?? []).length === 0 ? (
                        <tr>
                          <td colSpan={5} className="px-4 py-14">
                            <div className="flex flex-col items-center text-center">
                              ...
                              <p className="mb-1 font-semibold text-white">Sem keywords monitorizadas</p>
                              <p className="text-sm text-slate-400">Adicione palavras-chave para acompanhar o seu posicionamento no Google.</p>
                            </div>
                          </td>
                        </tr>
                      ) : (
                        (keywords ?? []).map((kw) => (
                          <tr key={kw.id} className="hover:bg-slate-800/30">
                            <td className="px-6 py-4 text-sm font-medium text-white">{kw.keyword}</td>
                            <td className="px-6 py-4">
                              {kw.position ? (
                                <span className={cn('inline-flex h-7 w-7 items-center justify-center rounded-full text-sm font-bold',
                                  kw.position <= 3 ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/40' : kw.position <= 10 ? 'bg-amber-500/20 text-amber-400 border border-amber-500/40' : 'bg-slate-800 text-slate-300'
                                )}>
                                  {kw.position}
                                </span>
                              ) : '—'}
                            ...
                            <td className="px-6 py-4">
                              {kw.url && (
                                <a href={kw.url} className="flex items-center gap-1 text-xs text-emerald-400 hover:underline" target="_blank" rel="noreferrer">
                            ...
                            <td className="px-4 py-4">
                              <button
                                ...
                                className="rounded p-1.5 text-slate-400 hover:bg-red-950/30 hover:text-red-400 disabled:opacity-40"
                              >
                                <Trash2 className="h-3.5 w-3.5" />
                              </button>
                            </td>
          ...
          {activeTab === 'analytics' && (
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
                {[
                  { label: 'Total de Campanhas', value: summary.total_campaigns ?? 0, icon: Mail },
                  { label: 'Total de Leads', value: summary.total_leads ?? 0, icon: TrendingUp },
                  { label: 'Leads Ganhos', value: summary.won_leads ?? 0, icon: CheckCircle2 },
                  { label: 'Taxa de Conversão', value: `${summary.conversion_rate ?? 0}%`, icon: Send },
                ].map((stat) => {
                  const Icon = stat.icon
                  return (
                    <div key={stat.label} className="rounded-xl border border-slate-800/80 bg-[#0f1422] p-5">
                      <div className="mb-3 flex h-9 w-9 items-center justify-center rounded-lg bg-emerald-500/20 text-emerald-400 border border-emerald-500/30">
                        <Icon size={18} />
                      </div>
